...
// each derived class can do as they want... example, set the current 
// explosion type for rpg or akimbo uzi etc.
...
void Weapon::AddMuzzleFlashEffect(){
    muzzleFlash.emplace_back(
        std::make_unique<MuzzleFlash>(MuzzleFlash::GetNextFlash(),muzzlePosition,relative.normalized(),.3f));
}
